...
def color_threshold(img, s_thresh=(0,255),l_thresh=(0,255)):
...
for index, fname in enumerate(images):
    ...
    gradx = abs_sobel_thresh(img, orient='x', abs_thresh=(20,220))
    grady = abs_sobel_thresh(img, orient='y', abs_thresh=(0,220))
    c_binary = color_threshold(img, s_thresh=(15,225), l_thresh=(50,225)) #100,255 & 50,255
...
#40:00
    ...
    src = np.array([[p0],[p1],[p2],[p3]], np.int32)

    result = preprocessImage
    write_name = './test_images/processed_'+str(index)+'.jpg'
    cv2.imwrite(write_name,result)
    print('saved processed files')

    # draw polygon region of interest, green line with thickness of 5 px
    src = src.reshape((-1,1,2))
    cv2.polylines(img, [src], True, (0,255,0))
    result = img
    write_name = './test_images/ROI_'+str(index)+'.jpg'
    cv2.imwrite(write_name,result)
    print('saved ROI files')
    ...
    result = warped
    write_name = './test_images/warped_'+str(index)+'.jpg'
    cv2.imwrite(write_name,result)
    print('saved warped files')

    #original (25,80)
    window_width = 25
    window_height = 80

    curve_centers = tracker(Mywindow_width = window_width, Mywindow_height = window_height, Mymargin = 25, My_xm = 10/720, My_ym = 4/384, Mysmooth_factor = 15) #10,4
    #mters per pixel
    ...
    warpage = np.array(cv2.merge((warped, warped, warped)), np.uint8) #stacking R,G,B channels, making original road pixels 3 color channels
    result = cv2.addWeighted(warpage, 1, template, 0.5, 0.0) # overlay original road images with window results

    write_name = './test_images/Green_'+str(index)+'.jpg'
    cv2.imwrite(write_name,result)
    print('saved green files')
    ...
    right_fitx = right_fit[0] * yvals + right_fit[1] * yvals + right_fit[2]
    right_fitx = np.array(right_fitx, np.int32)

    left_fit = np.polyfit(res_yvals, leftx, 2)
    left_fitx = left_fit[0] * yvals + left_fit[1] * yvals + left_fit[2]
